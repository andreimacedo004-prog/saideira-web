...
import SeletorDeCervejas, { type CervejaEscolhida } from "../componentes/SeletorDeCervejas";
import { enviarFoto, fotosAtivas } from "../fotos";
import { useRequisicao } from "../ganchos/useRequisicao";
import { FORMATOS_PADRAO, TIPOS_DE_ROLE } from "../rotulos";
import { paraDataHoraLocal } from "../tempo";
import type { TipoRole } from "../tipos";
...
export default function NovoCheckIn() {
  ...
  const [tipo, setTipo] = useState<TipoRole>("BAR");
  ...
  async function aoEnviar(evento: FormEvent) {
    ...
    try {
      ...
      setEtapa("salvando");
      const criado = await fazerCheckIn(desafioId, {
        tipo,
        local: local.trim(),
        fotoUrl,
        legenda: legenda.trim() || undefined,
        feitoEm: foiMaisCedo ? feitoEm : undefined,
        amigosIds: amigos,
  ...
  return (
    <form className="pagina checkin-form" onSubmit={aoEnviar}>
      ...
      <h1 className="titulo">Check-in</h1>

      {fotosAtivas ? (
        <label className={previa ? "foto-campo foto-campo--com-foto" : "foto-campo"}>
          {previa ? <img src={previa} alt="Prévia da foto" /> : <span>📸 Tirar ou escolher foto</span>}
      ...
      <button className="botao botao--largo botao--grande" type="submit" disabled={ocupado}>
        {etapa === "enviando-foto" ? "Enviando foto…" : etapa === "salvando" ? "Salvando…" : "Fazer check-in 🍻"}
      </button>
    </form>
  );
}
